...
class TagsUI extends PluginBase
{
	...
	public function TagsUI(Player $player) : void{
		...
		switch($result) {
			...
			case 1:
			$ppchat = $this->plugin->getServer()->getPluginManager()->getPlugin("PureChat");
			$ppchat->setSuffix((TextFormat::AQUA . "§8§dLUCID§a:§2CRAFT§8"), $player);
			$player->sendMessage(TextFormat::GREEN . "Set Tag To " . TextFormat::AQUA . "LUCIDCRAFT");
			break;	
			case 2:
			$ppchat = $this->plugin->getServer()->getPluginManager()->getPlugin("PureChat");
		...
		$form->setContent(TextFormat::AQUA . "Choose a tag!");
		$form->addButton(TextFormat::GREEN . "VOTER");
		$form->addButton(TextFormat::GREEN . "LUCID:CRAFT");
